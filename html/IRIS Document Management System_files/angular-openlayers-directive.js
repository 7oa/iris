/** angular-openlayers directive v1.19.0 from 2017-02-10
 * @SEE https://github.com/tombatossals/angular-openlayers-directive
 * when updating please consider the code that was changed by ITC (search for '@CHANGED_BY_ITC')
 */
                ...
                olScope.getMap().then(function (map) {
                    ...
                    scope.$watch('properties', function (properties, oldProperties) {
                        ...
                        if (!isDefined(olLayer)) {
                            ...
                            // set style
                            if (isDefined(properties.style) && !equals(properties.style, oldProperties.style) || isNewLayer(olLayer)) {
                                if (!angular.isFunction(properties.style)) {
                                    style = createStyle(properties.style);
                                } else {
                                    style = properties.style;
            ...
            switch (source.type) {
                ...
                case 'TileImage':
                    oSource = new ol.source.TileImage({
                        url: source.url,
                        /**
                         * @CHANGED_BY_ITC cross origin mode to support frontend separation
                         */
                        crossOrigin: 'anonymous',
                        attributions: createAttribution(source),
                        tileLoadFunction: source.tileLoadFunction,
            ...
            createLayer: function (layer, projection, name, onLayerCreatedFn) {
                ...
                for (var property in layer) {
                    if (layer.hasOwnProperty(property) &&
                        ...
                        !property.startsWith('$') && !property.startsWith('source') && !property.startsWith('style')
                    ) {
    ...
    angular.module('openlayers-directive').factory('olMapDefaults', ["$q", "olHelpers", function ($q, olHelpers) {
        ...
        var _getDefaults = function () {
            return {
                ...
                events: {
                    /**
                     * @CHANGED_BY_ITC added 'singleclick', 'pointermove'
                     */
                    map: ['singleclick', 'pointermove'],
                    markers: [],
                    /**
                     * @CHANGED_BY_ITC added 'click'
                     */
                    layers: ['click']
                },
                controls: {
                    ...
                    zoom: true,
                    /**
                     * @CHANGED_BY_ITC added fullscreen: true
                     */
                    fullscreen: true
                },
                interactions: {
                    /**
                     * @CHANGED_BY_ITC to true
                     */
                    mouseWheelZoom: true
                },
                renderer: 'canvas'
